...
class Canny_CNN(nn.Module):
    def __init__(self, sigma=1.0, high_threshold=0.2, low_threshold=0.1, use_quantiles=False):
        super(Canny_CNN, self).__init__()
    ...
    def forward(self, x):
        ...
        # --------- Find local maxima --------------
        #
        local_maxima = torch.zeros(isobel.shape).byte().to(x.device)
        # ----- 0 to 45 degrees ------
        pts_plus = (isobel >= 0) & (jsobel >= 0) & (abs_isobel >= abs_jsobel)
        pts_minus = (isobel <= 0) & (jsobel <= 0) & (abs_isobel >= abs_jsobel)
        pts = pts_plus | pts_minus
