...
parser.add_argument('--supdata', default='/mnt/lustrenew/yanglei/haonan/data/cinic_imgnet15000', type=str)
# Acceleration
...
train_loader = torch.utils.data.DataLoader(
    train_data, batch_size=args.batch_size//4, shuffle=True,
    num_workers=args.prefetch, pin_memory=torch.cuda.is_available())
test_loader = torch.utils.data.DataLoader(
    test_data, batch_size=args.test_bs, shuffle=False,
    num_workers=args.prefetch, pin_memory=torch.cuda.is_available())
sup_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(args.supdata+'/train', transform=train_transform), batch_size=args.batch_size//4*3, shuffle=True,
    num_workers=args.prefetch, pin_memory=torch.cuda.is_available())
...
def train():
    ...
    for bx, by in train_loader:
        ...
        adv_sup_bx = adversary(net, sup_bx, sup_by)
        sup_logits = net(adv_sup_bx * 2 - 1)

        # backward
        # scheduler.step()
        loss_train = F.cross_entropy(logits, by)
        loss_sup = F.cross_entropy(sup_logits, sup_by)
        loss = loss_train + loss_sup * 0.3
        loss.backward()
        optimizer.step()
...
with open(os.path.join(args.save, args.dataset + args.model +
                                  '_adv_training_results.csv'), 'w') as f:
...
for epoch in range(0, args.epochs):
    ...
    with open(os.path.join(args.save, args.dataset + args.model +
                                      '_adv_training_results.csv'), 'a') as f:
