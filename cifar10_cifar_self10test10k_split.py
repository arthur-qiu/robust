...
# ten_transform = trn.Compose([trn.Pad(4), trn.TenCrop(32),
#                                Lambda(lambda crops: torch.stack([trn.ToTensor()(crop) for crop in crops]))])


train_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(args.dataroot+'/cinic-10-cifar/train10k3', transform=test_transform), batch_size=8, shuffle=False,
    num_workers=args.prefetch, pin_memory=torch.cuda.is_available())
test_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(args.dataroot+'/cinic-10-cifar/test', transform=test_transform), batch_size=8, shuffle=False,
    num_workers=args.prefetch, pin_memory=torch.cuda.is_available())
sup_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(args.dataroot+'/cinic-10-cifar/train10k4', transform=test_transform), batch_size=8, shuffle=False,
    num_workers=args.prefetch, pin_memory=torch.cuda.is_available())
...
opt_alpha = opt_combine.PGD_combine_10trs(epsilon = 0.5, num_steps = 10, step_size = 0.1).cuda()
...
def mix_dataset():
    ...
    for class_name in classes:
        if not os.path.exists('/mnt/lustrenew/yanglei/haonan/data/cinic-10-cifar/self10combine/'+class_name):
            os.makedirs('/mnt/lustrenew/yanglei/haonan/data/cinic-10-cifar/self10combine/'+class_name)
    count = 0
    loss_avg = 0.0
    correct = 0
    for bx, by in train_loader:
        ...
        new_bx = opt_alpha(predict_net, first_bx, second_bx, first_by)
        logits = net(new_bx * 2 - 1)
        loss = F.cross_entropy(logits, first_by)
        loss_avg += float(loss.data)

        pred = logits.data.max(1)[1]
        correct += pred.eq(first_by.data).sum().item()

        for j in range(new_bx.shape[0]):
            count += 1
            np_img = (new_bx[j].cpu().numpy().transpose(1, 2, 0)*255).astype(np.uint8)
            img = Image.fromarray(np_img)
            img.save('/mnt/lustrenew/yanglei/haonan/data/cinic-10-cifar/self10combine/'+classes[by[0].data]+'/'+str(count)+'.png')

    for bx, by in train_loader:
        bx, by = bx.cuda(), by.cuda()

        indices1 = torch.LongTensor([0, 2, 4, 6]).cuda()
        indices2 = torch.LongTensor([1, 3, 5, 7]).cuda()
        first_bx = torch.index_select(bx,0,indices1)
        first_by = torch.index_select(by,0,indices1)
        second_bx = torch.index_select(bx,0,indices2)
    ...
    state['test_loss'] = loss_avg / len(train_loader)
    state['test_accuracy'] = correct / len(train_loader.dataset)
    print(state)
...
def train():
    ...
    for bx, by in train_loader:
        ...
        sup_bx = sup_bx.cuda()
        sup_predicts = predict_net(sup_bx * 2 - 1)
        sup_pred = torch.argmax(sup_predicts, 1)

        both_bx = torch.cat((bx, sup_bx), 0)
        both_by = torch.cat((by, sup_pred), 0)

        net.eval()
        adv_bx = adversary(net, both_bx, both_by)
        net.train()
...
def test_in_trainset():
    train_loader = torch.utils.data.DataLoader(
        dset.ImageFolder(args.dataroot + '/cinic-10-cifar/train10k3', transform=test_transform),
