...
train_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(args.dataroot+'/cinic-10-cifar/train10k', transform=train_transform), batch_size=args.batch_size//2, shuffle=True,
...
sup_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(args.dataroot+'/cinic_imgnetdivide/train10k', transform=train_transform), batch_size=args.batch_size//2, shuffle=True,
    num_workers=args.prefetch, pin_memory=torch.cuda.is_available())
...
ad_optimizer = torch.optim.SGD(
    ad_net.parameters(), state['learning_rate'], momentum=state['momentum'],
    weight_decay=state['decay'], nesterov=True)
...
def train():
    ...
    for bx, by in train_loader:
        ...
        target_classifier_loss = nn.CrossEntropyLoss()(outputs_target, target_predicts_pred)
        loss = transfer_loss * args.feature_weight + classifier_loss + target_classifier_loss
        loss.backward()
        optimizer.step()
        ad_optimizer.step()
...
def test_in_testset():
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            # forward
            adv_feature, adv_output = net(adv_data * 2 - 1)
...
for epoch in range(0, args.epochs):
    ...
    if epoch in epoch_step:
        ...
        optimizer = torch.optim.SGD(
            ...
            weight_decay=state['decay'], nesterov=True)
        ad_optimizer = torch.optim.SGD(
            ad_net.parameters(), lr, momentum=state['momentum'],
            weight_decay=state['decay'], nesterov=True)
