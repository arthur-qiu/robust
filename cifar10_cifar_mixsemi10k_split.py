...
train_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(args.dataroot+'/cinic-10-cifar/train10k3', transform=test_transform), batch_size=args.batch_size//2, shuffle=True,
...
sup_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(args.dataroot+'/cinic-10-cifar/train10k4', transform=test_transform), batch_size=args.batch_size//2, shuffle=True,
    num_workers=args.prefetch, pin_memory=torch.cuda.is_available())
...
def mix_dataset():
    ...
    for class_name in classes:
        if not os.path.exists('/mnt/lustrenew/yanglei/haonan/data/cinic-10-cifar/combine/'+class_name):
            os.makedirs('/mnt/lustrenew/yanglei/haonan/data/cinic-10-cifar/combine/'+class_name)
    count = 0
    loss_avg = 0.0
    ...
    for bx, by in train_loader:
        ...
        new_bx = opt_alpha(predict_net, bx, sup_bx)
        logits = net(new_bx * 2 - 1)
        new_pred = torch.argmax(logits, 1)
        loss = F.cross_entropy(logits, new_pred)
        ...
        for j in range(new_bx.shape[0]):
            ...
            img.save('/mnt/lustrenew/yanglei/haonan/data/cinic-10-cifar/combine/'+classes[count%10]+'/'+str(count)+'.png')
