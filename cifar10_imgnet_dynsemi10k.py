...
def train():
    ...
    for bx, by in train_loader:
        ...
        # scheduler.step()
        optimizer.zero_grad()
        if args.sup_weight < 1.0:
            loss_a = F.cross_entropy(logits[:logits.shape[0]//2], both_by[:both_by.shape[0]//2])
            loss_b = F.cross_entropy(logits[logits.shape[0]//2:], both_by[both_by.shape[0]//2:])
            loss = (loss_a + loss_b * args.sup_weight) / (1+args.sup_weight)
        else:
