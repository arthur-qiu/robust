...
parser.add_argument('--epoch_step', default='[30,32,34,36,38,40,42,44,46,48,50,52,54,56,58]', type=str,
...
sup_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(args.dataroot+'/cinic-10-cifar/gan50k_s0', transform=train_transform), batch_size=args.batch_size//2, shuffle=True,
    num_workers=args.prefetch, pin_memory=torch.cuda.is_available())
...
def train():
    ...
    for bx, by in train_loader:
        ...
        # scheduler.step()
        optimizer.zero_grad()
        # loss_a = F.cross_entropy(logits[:bx.shape[0]], both_by[:bx.shape[0]])
        # loss_b = F.cross_entropy(logits[bx.shape[0]:], both_by[bx.shape[0]:])
        # loss = loss_a + loss_b
