...
train_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(args.dataroot+'/cinic-10-cifar/train10k', transform=test_transform), batch_size=args.batch_size, shuffle=False,
...
sup_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(args.dataroot+'/cinic_imgnetdivide/train10k', transform=test_transform), batch_size=args.batch_size, shuffle=False,
    num_workers=args.prefetch, pin_memory=torch.cuda.is_available())
...
def test_in_trainset():
    train_loader = torch.utils.data.DataLoader(
        dset.ImageFolder(args.dataroot + '/cinic-10-cifar/selfcombine', transform=test_transform),
...
def label_in_supset():
    ...
    for class_name in classes:
        if not os.path.exists('/home/xiaocw/datablaze3/haonan/cinic_imgnet5000/predict_train/'+class_name):
            os.makedirs('/home/xiaocw/datablaze3/haonan/cinic_imgnet5000/predict_train/'+class_name)
    with torch.no_grad():
        for data, target in sup_loader:
            data, target = data.cuda(), target.cuda()
            ...
            for j in range(data.shape[0]):
                count += 1
                np_img = (data[j].cpu().numpy().transpose(1, 2, 0) * 255).astype(np.uint8)
                img = Image.fromarray(np_img)
                img.save(
                    '/home/xiaocw/datablaze3/haonan/cinic_imgnet5000/predict_train/' + classes[pred[j].data] + '/' + str(
                        count) + '.png')

    state['train_loss'] = loss_avg / len(sup_loader)
    state['train_accuracy'] = correct / len(sup_loader.dataset)
    state['adv_train_loss'] = adv_loss_avg / len(sup_loader)
    state['adv_train_accuracy'] = adv_correct / len(sup_loader.dataset)
