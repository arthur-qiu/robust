...
def vis_test():
    ...
    net_zip.eval()
    net.eval()
    two_nets = TwoNets(net_zip, net).cuda()
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            # forward
            adv_output = two_nets(adv_data * 2 - 1)
...
# def vis_canny():
#
#     if not os.path.exists(args.save + "/edge_canny/"):
#         os.makedirs(args.save + "/edge_canny/")
#
#     net_zip.eval()
#     net.eval()
#     two_nets = TwoNets(net_zip, net).cuda()
...
#                 image_numpy = np.clip(image_numpy.astype(np.uint8), 0, 255)
#                 single_image = Image.fromarray(image_numpy)
#                 single_image.save(args.save + "/edge_canny/" + str(single) + ".png")
#
#                 image_tensor = adv_zip_map[single].repeat(3, 1, 1)
#                 image_numpy = image_tensor.cpu().float().numpy()
#                 image_numpy = (np.transpose(image_numpy, (1, 2, 0)) + 1) / 2.0 * 255.0
#                 image_numpy = np.clip(image_numpy.astype(np.uint8), 0, 255)
#                 single_image = Image.fromarray(image_numpy)
#                 single_image.save(args.save + "/edge_canny/" + str(single) + "_adv.png")
#
#                 image_numpy = data[single].cpu().float().numpy()
#                 image_numpy = np.transpose(image_numpy, (1, 2, 0)) * 255.0
#                 image_numpy = np.clip(image_numpy.astype(np.uint8), 0, 255)
#                 single_image = Image.fromarray(image_numpy)
#                 single_image.save(args.save + "/edge_canny/" + str(single) + "_ori.png")
#
#                 image_numpy = data[single].cpu().float().numpy()
#                 image_numpy = np.transpose(image_numpy, (1, 2, 0))
#                 img_gray = image_numpy[...,0] * 0.299 + image_numpy[...,1] * 0.587 + image_numpy[...,2] * 0.114
#                 edge = canny(img_gray, sigma=1, high_threshold=0.2,
#                              low_threshold=0.1, thres=0.0).astype(np.float)
#                 image_numpy = edge * 255.0
#                 image_numpy = np.clip(image_numpy.astype(np.uint8), 0, 255)
#                 single_image = Image.fromarray(image_numpy)
#                 single_image.save(args.save + "/edge_canny/" + str(single) + "_canny0.png")
#
#             break
...
def vis_canny():
    ...
    net_zip.eval()
    net.eval()
    # canny_net = Canny_Net(use_cuda=True).cuda()
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            for single in range(16):
                ...
                # edge = canny_net(data[single:single+1])
                # image_numpy = edge[0].repeat(3, 1, 1).cpu().float().numpy()
                image_numpy = edge[single].repeat(3, 1, 1).cpu().float().numpy()
                image_numpy = np.transpose(image_numpy, (1, 2, 0)) * 255.0
                image_numpy = np.clip(image_numpy.astype(np.uint8), 0, 255)
                single_image = Image.fromarray(image_numpy)
                single_image.save(args.save + "/edge_canny/" + str(single) + "_canny.png")
