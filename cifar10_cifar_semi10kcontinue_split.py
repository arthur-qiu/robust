...
train_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(args.dataroot+'/cinic-10-cifar/train10k3', transform=train_transform), batch_size=args.batch_size//2, shuffle=True,
...
sup_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(args.dataroot+'/cinic-10-cifar/train10k4', transform=train_transform), batch_size=args.batch_size//2, shuffle=True,
    num_workers=args.prefetch, pin_memory=torch.cuda.is_available())
...
def train():
    ...
    for bx, by in train_loader:
        bx = bx.cuda()
        predicts = predict_net(bx * 2 - 1)
        pred = torch.argmax(predicts, 1)
        ...
        both_by = torch.cat((pred, sup_pred), 0)
