...
train_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(args.dataroot+'/cinic-10-cifar/train', transform=train_transform), batch_size=args.batch_size//2, shuffle=True,
...
sup_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(args.dataroot+'/cinic_imgnetdivide/train50k', transform=train_transform), batch_size=args.batch_size//2, shuffle=True,
    num_workers=args.prefetch, pin_memory=torch.cuda.is_available())
...
def test_in_testset():
    ...
    state['adv_test_loss'] = adv_loss_avg / len(train_loader)
    state['adv_test_accuracy'] = adv_correct / len(train_loader.dataset)


def test_in_trainset():
    train_loader = torch.utils.data.DataLoader(
        dset.ImageFolder(args.dataroot + '/cinic-10-cifar/train', transform=test_transform),
    ...
    state['train_loss'] = loss_avg / len(test_loader)
    state['train_accuracy'] = correct / len(test_loader.dataset)
    state['adv_train_loss'] = adv_loss_avg / len(test_loader)
    state['adv_train_accuracy'] = adv_correct / len(test_loader.dataset)
