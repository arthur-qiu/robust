...
test_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(args.dataroot+'/cinic-10-cifar/test', transform=ten_transform), batch_size=args.test_bs, shuffle=False,
    num_workers=args.prefetch, pin_memory=torch.cuda.is_available())
sup_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(args.dataroot+'/cinic-10-cifar/valid10k', transform=ten_transform), batch_size=args.batch_size, shuffle=False,
    num_workers=args.prefetch, pin_memory=torch.cuda.is_available())
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            for i in range(data.shape[0]):
                y = target[i:i+1].repeat(10)
                one_data = data[i]
                adv_data = adversary(net, one_data, y)
                output = net(adv_data * 2 - 1)
                pred = output.data.max(1)[1]
                correct_number = pred.eq(y.data).sum().item()
